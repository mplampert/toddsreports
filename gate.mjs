...
export async function revenueByDay(startYmd, endYmd, tz = "America/New_York", opts = {}) {
  ...
  while (guard++ < 4000) {
    ...
    for (const n of nodes) {
      ...
      if (!invoiced) {
        // Payments add; Refunds subtract (net revenue, matching Printavo).
        // Other union members (Return/Expense) never have transactionDate in
        // our query, so they were already skipped by the `if (!ts)` guard above.
