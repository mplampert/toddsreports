...
/**
 * Normalize a Printavo timestamp to a "YYYY-MM-DD" bucket.
 * Printavo's invoice createdAt comes back as a PLAIN DATE ("2025-01-07", no
 * time). Running a date-only value through a timezone conversion shifts it back
 * a day, so if it's already date-only we use it verbatim; only full timestamps
 * get the timezone treatment.
 */
